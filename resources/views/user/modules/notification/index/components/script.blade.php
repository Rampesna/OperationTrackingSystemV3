<script>
    ...
    function sendNotification() {
        $('#send_notification_heading').val('');
        $('#send_notification_message').val('');
    ...
    SendNotificationButton.click(function () {
        ...
        var heading = $('#send_notification_heading').val();
        var message = $('#send_notification_message').val();

        if (employeeIds.length === 0) {
            toastr.warning('Hiç Personel Seçmediniz!');
        } else if (!heading) {
            toastr.warning('Başlık Boş Bırakılamaz!');
        } else if (!message) {
            toastr.warning('Mesaj Boş Bırakılamaz!');
        } else {
            SendNotificationButton.attr('disabled', true).html('<i class="fa fa-spinner fa-spin"></i>');
            $.ajax({
                type: 'post',
                url: '{{ route('user.api.notification.sendBatch') }}',
                headers: {
                    'Accept': 'application/json',
                    'Authorization': token
                },
                ...
                success: function () {
                    toastr.success('Bildirimler Gönderildi!');
                    SendNotificationButton.attr('disabled', false).html('Gönder');
                    $('#SendNotificationModal').modal('hide');
                },
                error: function (error) {
                    console.log(error);
                    toastr.error('Bildirimler Gönderilirken Serviste Bir Hata Oluştu!');
                    SendNotificationButton.attr('disabled', false).html('Gönder');
                }
            });
        }
